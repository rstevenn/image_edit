...
size_t pull_window_event(app_state_t* app_data, call_back_t* call_back) {
    ...
    // wait mutex
    CHECK(WaitForSingleObject(app_data->win_event_accest_mtx, WAIT_TIMEOUT) == 0, "wait mutex failed")
    INFO("got mutex")
    ...
    // release
    CHECK(ReleaseMutex(app_data->win_event_accest_mtx) != 0, "Failed to release mutex");
    INFO("Mutex released")
    return out+1;
